        ...
        <div className="px-5 flex flex-col gap-3">
          <Card className="mb-1">
            <p className="text-xs leading-relaxed" style={{ color: theme.textMuted }}>
              Sezione segnaposto: le schede sotto sono esempi di struttura. Saranno sostituite da dati reali sulle prossime uscite navi quando colleghiamo la fonte ufficiale (Fase 3).
            </p>
          </Card>
          {UPCOMING_SHIPS.map((s) => (
            <Card key={s.id}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[11px]" style={{ color: theme.cyan }}>{s.manufacturer.toUpperCase()} · {s.status}</p>
                  <p className="font-semibold text-sm mt-0.5">{s.name}</p>
                  <p className="text-xs mt-1" style={{ color: theme.textMuted }}>{s.note}</p>
                </div>
  ...
  return (
    <div className="pb-28">
      ...
      <div className="px-5 flex flex-col gap-3">
        <Card className="mb-1">
          <p className="text-xs leading-relaxed" style={{ color: theme.textMuted }}>
            Contenuto segnaposto: sarà sostituito dal changelog ufficiale reale quando colleghiamo la fonte dati (Fase 3).
          </p>
        </Card>
        {CHANGELOG.map((c) => (
          <Card key={c.id} onClick={() => setOpenChangelog(openChangelog === c.id ? null : c.id)}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
